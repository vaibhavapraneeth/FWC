...
sys.path.insert(0,'/home/vaibhava_108/fwc/CoordGeo/CoordGeo')
#local imports
...
#Input parameters
l1=1
V = np.loadtxt('dat/v.dat',dtype :='float')
u = np.loadtxt('dat/u.dat',dtype :='float')
f = 0
lamda,P = LA.eigh(V)
...
vert_labels = ['A(8,-4)','B(-8,-4)','F(0,-4)','O(0,0)']
...
label = "{}{:.1f}".format("y = ", c) #Form label as A(x,y)
plt.annotate(label, # this is the text
             (plot_coords[0,2]-8, plot_coords[1,2]-3), # this is the point to label
             textcoords="offset points", # how to position the text
             xytext=(18,5), # distance from text to points (x,y)
             rotation = 0,
             ha='center') # horizontal alignment can be left, right or center
...
plt.title('Parabola')
#if using termux
#plt.savefig('/sdcard/FWC/conics/conic1/figs/parabola.png')
...
#else
print(f"Focus : {F}")
print(f"equation of directrix : y = {c}")
print(f"latus rectum : {lr}")
plt.show()
(f"equation of directrix : y = {c}")
print(f"latus rectum : {lr}")
plt.show()
